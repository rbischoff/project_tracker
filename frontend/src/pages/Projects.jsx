  ...
  return (
    <Card onClick={onClick} style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
        <div style={{ flex: 1 }}>
          <h3 style={{ fontSize: 17, marginBottom: 4, color: 'var(--charcoal)' }}>{project.name}</h3>
          <p style={{ fontSize: 12, color: 'var(--muted)', textTransform: 'capitalize' }}>{project.category.replace('_', ' ')}</p>
        </div>
      ...
      {project.description && (
        <p style={{ fontSize: 13, color: 'var(--slate)', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{project.description}</p>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, borderTop: '1px solid var(--border)', paddingTop: 12 }}>
        <div>
          <p style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 2 }}>Est. Cost</p>
          <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--charcoal)' }}>{formatCurrency(project.estimated_cost)}</p>
        </div>
        <div>
          <p style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 2 }}>Materials</p>
          <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--charcoal)' }}>{purchased}/{materials.length}</p>
        </div>
        <div>
          <p style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 2 }}>Target</p>
          <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--charcoal)' }}>{formatDate(project.target_date)}</p>
        </div>
      </div>
    </Card>
  );
...
export default function Projects() {
  ...
  useEffect(() => {
    api.getProjects(filters).then(p => { setProjects(p); setLoading(false); });
  }, [filters]);
    ...
    <div style={{ maxWidth: 1100, animation: 'fadeIn 0.3s ease' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          ...
          <h1 style={{ fontSize: 36, color: 'var(--charcoal)' }}>Projects</h1>
        </div>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: '1 1 200px', background: 'var(--cream)', borderRadius: 8, padding: '8px 12px', border: '1px solid var(--border)' }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search projects..."
              style={{ border: 'none', background: 'transparent', outline: 'none', fontSize: 14, color: 'var(--charcoal)', flex: 1 }} />
          </div>
          ...
          <div style={{ display: 'flex', gap: 4, marginLeft: 'auto' }}>
            {['grid', 'list'].map(m => (
              <button key={m} onClick={() => setViewMode(m)} style={{
                padding: '7px 10px', borderRadius: 6, border: '1px solid var(--border-dark)',
                background: viewMode === m ? 'var(--charcoal)' : 'transparent',
                color: viewMode === m ? 'white' : 'var(--muted)', cursor: 'pointer', display: 'flex',
              }}>
        ...
        <Card style={{ overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead style={{ background: 'var(--cream)' }}>
              <tr>
                {['Name', 'Priority', 'Status', 'Progress', 'Est. Cost', 'Category', 'Target Date'].map(h => (
            ...
            <tbody>
              {filtered.map(p => (
                <tr key={p.id} onClick={() => navigate(`/projects/${p.id}`)} style={{ borderBottom: '1px solid var(--border)', cursor: 'pointer' }}
                  ...
                  <td style={{ padding: '12px 16px', fontWeight: 500, color: 'var(--charcoal)' }}>{p.name}</td>
                  ...
                  <td style={{ padding: '12px 16px', textTransform: 'capitalize', color: 'var(--slate)' }}>{p.category.replace('_', ' ')}</td>
