...
const DEFAULT_FORM = {
  name: '', description: '', priority: 'medium', status: 'planned', category: 'general',
  estimated_cost: '', actual_cost: 0, progress: 0, start_date: '', target_date: '', notes: '', materials: [],
};
  ...
  return (
    <div style={{ maxWidth: 720, animation: 'fadeIn 0.3s ease' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <h1 style={{ fontSize: 36 }}>New Project</h1>
      </div>
          ...
          {form.materials.length === 0 ? (
            ...
            <>
              <div style={{ display: 'flex', gap: 8, padding: '0 0 8px', fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.06em', borderBottom: '1px solid var(--border)', marginBottom: 4 }}>
                <span style={{ width: 24 }} /><span style={{ flex: 2 }}>Name</span><span style={{ flex: 1 }}>Quantity</span><span style={{ flex: 1 }}>Cost ($)</span><span style={{ width: 30 }} />
              </div>
              {form.materials.map((m, i) => (
                <div key={i} style={{ display: 'flex', gap: 8, padding: '6px 0', alignItems: 'center' }}>
                  <input type="checkbox" checked={m.purchased} onChange={e => updateMaterial(i, 'purchased', e.target.checked)} style={{ width: 16, height: 16 }} />
                  <input value={m.name} onChange={e => updateMaterial(i, 'name', e.target.value)} placeholder="e.g. Ceramic tiles" style={{ flex: 2, padding: '6px 8px', border: '1px solid var(--border-dark)', borderRadius: 6, fontSize: 13 }} />
                  <input value={m.quantity} onChange={e => updateMaterial(i, 'quantity', e.target.value)} placeholder="e.g. 20 sq ft" style={{ flex: 1, padding: '6px 8px', border: '1px solid var(--border-dark)', borderRadius: 6, fontSize: 13 }} />
                  <input type="number" value={m.cost} onChange={e => updateMaterial(i, 'cost', parseFloat(e.target.value) || 0)} placeholder="0" style={{ flex: 1, padding: '6px 8px', border: '1px solid var(--border-dark)', borderRadius: 6, fontSize: 13 }} />
                  <button onClick={() => deleteMaterial(i)} style={{ background: 'none', border: 'none', color: 'var(--red)', cursor: 'pointer', padding: 4 }}><X size={15} /></button>
                </div>
              ))}
